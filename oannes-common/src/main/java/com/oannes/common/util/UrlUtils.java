...
public class UrlUtils {
                ...
                if (! serviceName.contains(":") && ! serviceName.contains("/")) {
                    for (URL url : subscribed) {
                        if (serviceName.equals(url.getServiceInterface())) {
                            newForbid.add(url.getServiceInterface());
                            break;
                        }
                    }
                } else {
    ...
    public static boolean isMatch(URL consumerUrl, URL providerUrl) {
        ...
        String consumerClassifier = consumerUrl.getParameter(Constants.CLASSIFIER_KEY);
        
        String providerGroup = providerUrl.getParameter(Constants.GROUP_KEY);
        String providerVersion = providerUrl.getParameter(Constants.VERSION_KEY);
        String providerClassifier = providerUrl.getParameter(Constants.CLASSIFIER_KEY);
        return  StringUtils.isEquals(consumerGroup, providerGroup) 
               &&   StringUtils.isEquals(consumerVersion, providerVersion)
               && consumerClassifier == null || StringUtils.isEquals(consumerClassifier, providerClassifier);
    }
